...
//在压缩存储中，对于-1如果不转换进行压缩，由于-1大部分位都是1，故需要5个字节进行存放，转换后，需要字节数减少
...
void ByteArray::writeUint32 (uint32_t value)
{
    uint8_t tmp[5];
    uint8_t i = 0;
    while(value >= 0x80)// 0x80		10000000
    {
		//0x7f	0111111 
...
{
    uint8_t tmp[10];//每8位中，只有前面7位存放数据，最后一位标志是否还有数据，所以存放64位的数据最多需要10个数组，7*9=63 <64 < 7*10 = 70
    uint8_t i = 0;
    //如果一个字节的最后一位为1表明后面还有数据
    while(value >= 0x80) {
        tmp[i++] = (value & 0x7F) | 0x80;
        value >>= 7;
    }
...
void ByteArray::write(const void* buf, size_t size)
{
    ...
    size_t npos = m_position % m_baseSize;//找到在某一块中的哪个位置
    size_t ncap = m_cur->size - npos;//记录当前内存块还能存放多少数据
    size_t bpos = 0;
...
//注意读的内存块链表和写的不是同一个
//读时，整个内存块链表总数据量是确定了的，只是会移动当前读到的位置
...
void ByteArray::read(void* buf, size_t size, size_t position) const
{
    ...
    size_t ncap = m_cur->size - npos;//当前块内存还剩多少数据未读
    size_t bpos = 0;
    Node* cur = m_cur;
    while(size > 0) 
	{
...
/*
调整容量，如果原本容量足够，则返回，如果不够，则可以计算还需多少内存块，申请新的内存快，加入链表
*/
