...
namespace sylar
{
	...
	int g_tcp_connect_timeout = 5000;
    //线程局部变量，记录当前线程是否hook
    ...
    void hook_init()
    {
        ...
        /*
        宏定义中，#str 表示将str视为一个字符串
        A##B 表示把两个宏变量进行拼接
        dlsym(void *handle, const char *symbol);从共享库中获取符号（全局变量与函数符号）地址
        handle可以是dl文件的open函数返回的handle值，也可以是RTLD_NEXT或RTLD_DEFAULT
        RTLD_NEXT：表示在当前库以后按默认的顺序搜索共享库中符号symbol第一次出现的地址
        RTLD_DEFAULT:表示按默认的顺序搜索共享库中符号symbol第一次出现的地址
        */
	...
	//定义一个静态变量，使得main()函数开始前，hook住相关函数
...
retry:
    ...
    //EINTR表示中断，当函数因中断而执行失败时，不断重新执行该函数
    while(n == -1 && errno == EINTR)
    {
        n = fun(fd, std::forward<Args>(args)...);
    }
    //EAGAIN表示因没有系统资源而阻塞，当函数因阻塞而执行失败时，会进行一些处理
    if(n == -1 && errno == EAGAIN) 
	{
        sylar::IOManager* iom = sylar::IOManager::GetThis();//获取当前IOManager
        sylar::Timer::ptr timer;//准备定时器
        std::weak_ptr<timer_info> winfo(tinfo);
        //当前socket设置了超时时间，则需要设置一个定时事件，当到达了超时时间后，需要将这个事件取消
        if(to != (uint64_t)-1)
        {
...
extern "C"
{
	...
	int socket(int domain, int type, int protocol)
    {
        ...
        sylar::FdMgr::GetInstance()->get(fd, true);//同步一下fdmanager里面的fd容器
        return fd;
    }
