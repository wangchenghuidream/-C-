...
bool Socket::getOption(int level, int option, void* result, socklen_t* len)
{
    ...
    int getsockopt(
    [in]    SOCKET s,
    [in]    int level,  //标识选项级别， SOL_SOCKET    套接字层次，  IPPROTO_IP  ip层次，   IPPROTO_TCP TCP层次
    [in]    int optname,  //SO_BROADCAST是否运行发送广播 SO_REUSEADDR是否允许重复使用本地地址   SO_SENDBUF发送缓冲区长度  SO_RCVBUF   SO_RCVTIMEO超时时间 SO_SENDTIMEO
    [out]   char *optval,
    [in, out]   int *optlen
                   );
    */
...
Socket::ptr Socket::accept()
{
    ...
    /*
    int accept(int sockt, struct sockaddr *addr, socklen_t *addrlen);
    返回一个新的套接字来和客户端通信，addr保存了客户端的ip地址和端口，sockt是服务器端的套接字
    */
...
{
	void *msg_name;
	socklen_t msg_namelen; 当socket是未连接状态时，用msg_name和msg_namelen指示source address
	struct iovec *msg_iov;	数据缓存数组
	size_t msg_iovlen;
	void *msg_control;可选的辅助数据的位置和大小
