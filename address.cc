...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,int family, int type, int protocol)
{
    ...
    {
        //void* memchr(void *buf, char c, int len); 从buf中查找len长度，找到c则返回指向该字符的指针，否则返回Null
...
{
    if(iface.empty() || iface == "*") //返回任意地址，0.0.0.0
    {
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port)
{
    ...
    /*
    int getaddrinfo(const char * restrict nodename, const char *restrict servname, const struct addrinfo *restrict hints, const addrinfo **restrict res)
    nodename: host或者ip地址
    servname:   十进制端口号 或者常用服务名称‘ftp’'http'
    hints:  获取信息要求设置，hints只能设置ai_familt\ai_socktype/ai_protocol/ai_flags
    res: 获取结果
    */
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port)
{
    ...
    /*
    int inet_pton(int family, const char *strptr, void *addrptr): 将点十分进制的ip地址转为网络传输的数值格式
    const char * inet_ntop(int family, void *addrptr, char *strptr, size_t len);将数值格式转化为点分十进制的ip地址格式
    */
...
UnixAddress::UnixAddress()
{
    ...
    //offsetof(s,m):能自动返回结构对象中，某变量距离结构体对象首地址的偏移值
    //s:结构体
    //m， 结构体s中的某个变量
